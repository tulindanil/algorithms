...
public:
    ...
    std::vector<Move> solve(const Grid &);
...
std::vector<Move> Worker::restorePath()
{
    Grid v = target;
    size_t length = 0;
    ...
    {
        length++;
        movements.push_back(moveMap[v]);
        assert(v.move(-moveMap[v].x, -moveMap[v].y));
    }
    
    return std::vector<Move>(movements.rbegin(), movements.rend());
}

std::vector<Move> Worker::solve(const Grid &source)
{
