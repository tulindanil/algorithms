...
int main()
{
    ...
    std::vector<std::pair<float, float> > cities, C;
    ...
    float min = FLT_MAX;
    for (std::vector<std::pair<float, float> >::const_iterator c = C.begin(); c != C.end(); ++c)
    {
        if (min > m[_find(A)][_find(*c)] + m[_find(*c)][_find(B)])
        {
            min = m[_find(A)][_find(*c)] + m[_find(*c)][_find(B)];
        }
    }
